if IsDuplicityVersion() then
    if GetResourceState(OXLibExport):find("start") then
        createCallback(GetCurrentResourceName() .. ':server:GetStashItems',
            function(source, stashName)
                local stash = getStash(stashName)
                return stash
            end)
    else
        createCallback(GetCurrentResourceName() .. ':server:GetStashItems',
            function(source, cb, stashName)
                local stash = getStash(stashName)
                cb(stash)
            end)
    end
...
function GetStashTimeout(stashName, stop)
    if stop then
        stashItems, timing, timeout = {}, false, 0
        return
    end
...
function craftingMenu(data)
            ...
            if k ~= "amount" and k ~= "job" and k ~= "gang" then
                ...
                if hasjob then
                    ...
                    for l, b in pairs(Recipes[i][tostring(k)]) do
                        settext = settext ..
                            (settext ~= "" and br or "") ..
                            (Items[l] and Items[l].label or "error - " .. l) .. (b > 1 and " x" .. b or "")
                        itemTable[l] = b
                        Wait(0)
                    end
                    while not canCarryTable do Wait(0) end
                    if Config.System.Debug then
                        print("^6Bridge^7: ^2Checking" ..
                            (data.stashName and " ^7'^6" .. data.stashName .. "^7'" or "") ..
                            " ^2ingredients^7 - ^6" .. k .. "^7")
                    end
                    ...
                    setheader = (Items[tostring(k)] and Items[tostring(k)].label or "error - " .. tostring(k)) ..
    ...
    openMenu(Menu,
        { header = data.craftable.Header, onBack = data.onBack or nil, canClose = true, onExit = function() end, })
    lookEnt(data.coords)
end
...
function multiCraft(data)
    ...
    for k in pairsByKeys(success) do
        ...
        if Config.System.Debug then
            print("^6Bridge^7: ^2Checking " ..
                (data.stashName and "^7'^6" .. data.stashName .. "^7'" or "inventory") ..
                "^7x^5" .. k .. " ^2ingredients^7 - ^6" .. data.item .. "^7")
        end
        if data.stashName then
            disable = not stashhasItem(stashItems, itemTable)
        else
...
function makeItem(data)
    ...
    local bartext = (data.craftable.progressBar and data.craftable.progressBar.label) or
        (Loc[Config.Lan].progressbar and Loc[Config.Lan].progressbar["progress_make"]) or "Making a"
                ...
                if type(v) == "table" then
                    ...
                    if crafted then
                        if crafting and progressBar({
                            ...
                            TriggerServerEvent(GetCurrentResourceName() .. ":Crafting:GetItem", data.item, data.craft,
                                data.stashName)
                        else
                            crafting = false
                            break
...
RegisterNetEvent(GetCurrentResourceName() .. "Sellitems", function(data)
    ...
    if hasItems then
        ...
        TriggerEvent(GetCurrentResourceName() .. ":server:FundPlayer", (hasTable[data.item].count * data.price), "cash",
            src)
    else
...
function openShop(data)
    ...
    if GetResourceState(OXInv):find("start") then
        ...
        TriggerServerEvent(Config.General.JimShops and "jim-shops:ShopOpen" or "inventory:server:OpenInventory", "shop",
...
function hasItem(items, amount, src)
    ...
    elseif GetResourceState(CodeMInv):find("start") then
        foundInv = CodeMInv
        if src then
            grabInv = exports[CodeMInv]:GetInventory(false, src)
        else
    ...
    if grabInv then
        ...
        for item, amount in pairs(items) do
            if not Items[item] then
                print("^4ERROR^7: ^2Script can't find ingredient item in Shared Items - ^1" ..
                    item .. "^7")
            end
            local count = 0
            ...
            local foundMessage = "^6Bridge^7: ^3hasItem^7[^6" ..
                foundInv .. "^7]: " .. tostring(item) .. " ^3" .. count .. "^7/^3" .. amount
            if count >= amount then
                foundMessage = foundMessage .. " ^5FOUND^7"
            else
...
function getStash(stashName)
    ...
    if stashItems then
        ...
        if Config.System.Debug then
            print("^6Bridge^7: ^3GetStashItems^7: ^2Stash information for ^7'^6" ..
                stashName .. "^7' ^2retrieved^7")
        end
    end
    return items
end
...
function stashRemoveItem(stashItems, stashName, items)
    ...
    elseif GetResourceState(QBInv):find("start") then
        ...
        MySQL.Async.insert(
            ...
            { ['stash'] = stashName, ['items'] = json.encode(stashItems) })
    else
        print("^4ERROR^7: ^2No Inventory detected ^7- ^2Check ^3exports^1.^2lua^7")
    end
end
...
function stashhasItem(stashItems, items, amount)
    ...
    for item, amount in pairs(items) do
        ...
        local debugMsg = string.format("^6Bridge^7: ^3stashHasItem^7[^6%s^7]: %s '%s' ^3%d^7/^3%d^7", foundInv,
            (count >= amount and "^5FOUND^7" or "^1NOT FOUND^7"), item, count, amount)
...
if IsDuplicityVersion() then
    if GetResourceState(OXLibExport):find("start") then
        createCallback(GetCurrentResourceName() .. ':server:canCarry',
            function(source, itemTable)
                local result = canCarry(itemTable, source)
                return result
            end)
    else
        createCallback(GetCurrentResourceName() .. ':server:canCarry',
            function(source, cb, itemTable)
                local result = canCarry(itemTable, source)
                cb(result)
            end)
    end
    ...
    if src then
        ...
        elseif GetResourceState(CodeMInv):find("start") then
            for k, v in pairs(itemTable) do
                ...
                resultTable[k] = exports[CodeMInv]:CanCarryItem(src, k, v)
            end
        elseif GetResourceState(OrigenInv):find("start") then
            for k, v in pairs(itemTable) do
        ...
        if reward then
            ...
            for i = 1, #Config.Rewards.RewardPool do
                ...
                if randomNum <= currentRarity then
                    if Config.System.Debug then
                        print("^6Bridge^7: ^3getRandomReward^7: ^2Selected toy ^7'^6" ..
                            Config.Rewards.RewardPool[i].item .. "^7'")
                    end
